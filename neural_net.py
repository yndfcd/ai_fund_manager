...
class Neuron(DifferentiableElement):
    ...
    def compute_output(self):
        ...
        """
        sum = 0.0
        for index in range(len(self.my_inputs)):
            i = self.my_inputs[index].output()
            w = self.my_weights[index].get_value()
            product = i * w
            sum += product
        return 1.0 / (1 + math.e ** -sum)
    ...
    def compute_doutdx(self, elem):
        ...
        if elem in self.my_weights:
            index =	list.index(self.my_weights, elem)
            return self.my_inputs[index].output() * o * (1 - o)
        else:
            sum = 0.0
            for index in range(len(self.my_inputs)):
                product = self.my_weights[index].get_value() * self.my_inputs[index].dOutdX(elem)
                sum += product
            return o * (1 - o) * sum 
